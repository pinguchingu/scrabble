...
// this method is added to return a bool value so that exceptions aren't used
// instead of logic
bool TileCollection::has_tile(char letter) const {
    for (TileMap::const_iterator it = tiles.begin(); it != tiles.end(); it++) {
        if (it->first.letter == tolower(letter)) {
            // if the letter is found in the collection return true
            return true;
        }
    }
